...
y_min = -5
y_max = 5
features = np.random.uniform(low=[x_min,y_min], high=[x_max,y_max], size=(100,2))

value_ = {'x':features[:,0], 'y':features[:,1]}
database = pd.DataFrame(value_)
print(database.tail())
database.to_csv('data/StyblinskiTang_100.csv',index=False)

data = open_data('data/StyblinskiTang_100.csv')
data = data.to_numpy()
train_data = data[:]
test_data = data[:]
...
for k in [10000,50000]:
    model = SobolevRegressor()
    out_S, out_sobolev_S = train_sobolev(model,train_loader,test_loader,1.0,nb_epochs=k,seed=123,save_model_dir='results/tang_100.pt',log_df='results/log_df.csv')
    model = SobolevRegressor()
    out1, out_sobolev1 = train_sobolev(model,train_loader,test_loader,0.0,nb_epochs=k,seed=123,save_model_dir='results/tangNN_100.pt',log_df='results/log_dfNN.csv')

    x,y = np.arange(-5,5,0.25),np.arange(-5,5,0.25)

    reg = SobolevRegressor()
    _device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    reg.load_state_dict(torch.load('results/tangNN_100.pt',map_location=torch.device(_device)))
    ...
    reg = SobolevRegressor()
    _device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    reg.load_state_dict(torch.load('results/tang_100.pt',map_location=torch.device(_device)))
    ...
    plot_results(100,k,'1e-4','1', out, out_sobolev)
